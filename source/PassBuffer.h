...
/**
    ...
    **It reallocates only when it has to.** Ensure() is called every frame and
    ...
    page geometry changes (Fit, Resolution, or the frame's aspect at a page
    start). The page itself lives on the CPU, so a reallocation clears nothing
    that matters.
    ...
    **It owns its filtering.** The scan buffer is packed bits read back to the
    CPU and must never be filtered, so it is Nearest. The other two modes are
    tinsel's and are unused here.

    Copied from tinsel (by way of teletext); the mechanism is unchanged.
*/
