...
| `source/Font.*`, `PassBuffer.*`, `Diag.*`, `StoatworksAbout*` | graticule's font; tinsel's buffer; the log; the About block (provisional). |
...
- **Superfine K = 8** is recalled from the Recommendation, not read from its text (no copy
  was fetched: this session downloads nothing). `--wedge` measures whatever `layout::K`
  returns; changing it is one line.
...
- **Superfine K = 8**, the error model and the paper colours are stated, not measured.
